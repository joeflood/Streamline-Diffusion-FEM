...
def solve(method, epsilon):
    ...
    if method == "SUPG2":
        if Peclet < 3:
            tau = dx/2 * (Peclet/3)  # SUPG stabilization parameter
        else:
            tau = dx/2
...
def plot(type, epsilon):
    def plotter_c(fill, epsilon, ax, method, title):
        X, Y, u_grid, x, y, grad_u = solve(method, epsilon)
        num_contours = 31
    ...
    def plotter3d(ax, epsilon, method, title):
    ...
    def plot_exact(ax, epsilon, method, title):
        X, Y, u_grid, x, y, grad_u = solve(method, epsilon)
    ...
    if type == "contourf":
        ...
        plotter3d(ax1, epsilon, method="Galerkin", title="Galerkin")
        plotter3d(ax2, epsilon, method="SUPG", title="SUPG")
        plot_exact(ax3, epsilon, method="SUPG", title = "Exact")

        plt.tight_layout()
        plt.show()

def exact_grad(x, y, epsilon):
    denom = 1 - np.exp(-2/epsilon)
    u_x = 3*x**2*(1-np.exp((y-1)/epsilon))/denom
    u_y = - (x**3 + 1) * np.exp((y - 1)/epsilon) / (epsilon * denom)
    return np.array([u_x,
                     u_y])
...
def compute_L2_grad_error_dblquad(grad_u, x, y, epsilon, epsabs=1e-8, epsrel=1e-6):
    ...
    result, err = dblquad(
        lambda yp, xp: integrand(yp, xp, grad_u, x, y, epsilon),
        xmin, xmax,
...
# Example usage:
for epsilon in [1/24, 1/36, 1/48, 1/60, 1/72, 1/96, 1/120]:
    X, Y, u_grid, x, y, grad_u = solve("Galerkin", epsilon)
    error_galerkin = compute_L2_grad_error_dblquad(grad_u, x, y, epsilon)

    X, Y, u_grid, x, y, grad_u = solve("SUPG1", epsilon)
    error_supg1 = compute_L2_grad_error_dblquad(grad_u, x, y, epsilon)

    X, Y, u_grid, x, y, grad_u = solve("SUPG2", epsilon)
    error_supg2 = compute_L2_grad_error_dblquad(grad_u, x, y, epsilon)

    print(epsilon, error_galerkin, error_supg1, error_supg2)